...
impl ChatWidget {
    ...
    pub(super) fn on_warning(&mut self, message: impl Into<String>) {
        ...
        // A `/shake` or `/smart-compact` completion notice arrived: release the pending input gate
        ...
        let message = if let Some(summary) = message
            .strip_prefix("⛭ shake: ")
            .or_else(|| message.strip_prefix("⛭ smart-compact: "))
        {
